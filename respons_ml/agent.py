...
def print_lim_reqs():
    """
    Create a prometheus client, connect to server, make queries, and print the calculated limits and requests for all pods
    Input: None
    Output: None
    """
    ...
    for i in range(len(max_cpu_data)):
        try:
            dict_lim_req[max_cpu_data[i]["metric"]["pod"]].append(
                max_cpu_data[i]["value"][1]
            )
            dict_lim_req[avg_cpu_data[i]["metric"]["pod"]].append(
                avg_cpu_data[i]["value"][1]
            )
            dict_lim_req[max_memory_data[i]["metric"]["pod"]].append(
                max_memory_data[i]["value"][1]
            )
            dict_lim_req[avg_memory_data[i]["metric"]["pod"]].append(
                avg_memory_data[i]["value"][1]
            )
        except KeyError:
